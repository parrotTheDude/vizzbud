...
<div class="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 text-slate-100 py-6 px-4 sm:px-6">
  <div class="max-w-6xl mx-auto space-y-10">
    ...
    {{-- 🧍 Diver Header --}}
    <section 
      class="relative flex flex-col bg-gradient-to-r from-slate-800/80 to-slate-900/60 
            border border-white/10 rounded-2xl p-6 sm:p-8 shadow-lg overflow-hidden">

      {{-- Decorative glow --}}
      <div class="absolute inset-0 bg-gradient-to-r from-cyan-500/10 via-transparent to-cyan-400/10 blur-2xl opacity-30 pointer-events-none"></div>

      {{-- Top Row: Avatar + Basic Info --}}
      <div class="flex items-center gap-4 sm:gap-6 relative z-10 flex-wrap sm:flex-nowrap">
        {{-- Avatar --}}
        <div class="relative shrink-0 group">
          <img 
            src="{{ $user->profile->avatar_url ?? asset('images/main/defaultProfile.webp') }}"
            alt="Profile photo"
            class="w-20 h-20 sm:w-28 sm:h-28 rounded-full object-cover border-2 border-cyan-400/40 shadow-md
                  transition-transform duration-300 group-hover:scale-105">
          <div class="absolute inset-0 rounded-full bg-cyan-500/20 opacity-0 group-hover:opacity-100 transition duration-300 blur-md"></div>
        </div>

        {{-- Info Block (right of avatar) --}}
        <div class="flex-1 min-w-[60%] sm:min-w-0">
      ...
      @if(!empty($user->profile->bio))
        <p class="text-slate-400 text-sm mt-4 italic leading-relaxed max-w-2xl relative z-10">
          “{{ Str::limit($user->profile->bio, 180) }}”
        </p>
      @endif
      ...
      <div class="mt-5 relative z-10">
        <a href="{{ route('profile.edit') }}"
          class="inline-flex items-center gap-2 rounded-full bg-cyan-600 hover:bg-cyan-500 
                  text-white px-5 py-2.5 text-sm font-semibold shadow-md shadow-cyan-600/30 
                  hover:shadow-cyan-500/30 transition-all duration-200 active:scale-[0.98] w-full sm:w-auto justify-center">
